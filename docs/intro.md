...
1. 目的是生成两个过程图：`BinaryImg`和`colorImg`。
   
2. 对原图像分为三通道split，为检测红色，将r通道减去b通道（检测蓝色则相反：将b通道减去r通道），得到的结果再进行二值化处理得到二值图（负值自动归零）。
   
3. 对图像进行灰度处理bgr2gray，然后进行归一化normalize。对归一化后图像进行二值化处理，并进行一次闭运算，除去近距离的小黑洞（务必不要使用开运算，删去远距离灯条！）。
